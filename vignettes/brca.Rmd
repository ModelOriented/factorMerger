...
```{r}
devtools::install_github("RTCGA/RTCGA", build_vignettes = TRUE)
library(RTCGA)
...
```{r}
data("BRCA.clinical")
colsToBeExtracted <- c("patient.drugs.drug.drug_name",
                       "patient.clinical_cqcf.histological_type",
                       "patient.stage_event.tnm_categories.pathologic_categories.pathologic_t")

BRCA <- survivalTCGA(BRCA.clinical,
             extract.cols = colsToBeExtracted)
BRCA <- dplyr::filter(BRCA, times > 0) 
```
