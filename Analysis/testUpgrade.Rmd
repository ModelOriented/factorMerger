...
apartmentsSubset$street <- recodeLevels(apartmentsSubset$transaction.price, 
                                        apartmentsSubset$street)
...
******
### ANOVA na przykładzie cen mieszkań
...
Liczba policzonych statystyk $t$/$p$-value w całym algorytmie: $\sum_{i = 1}^k {i \choose 2}$ (dużo -- sześciennie).
...
Liczba policzonych statystyk $t$/$p$-value w całym algorytmie: $\sum_{i = 1}^{k - 1} i$ (kwadratowo).
...
Minusy:
 ...
 - w każdym kroku na nowo liczymy model pomniejszony o jeden parametr.

Implementacja: kontrasty!
 
Obserwacja: w każdym kroku łączymy tylko jedną parę średnich $\mu_i, \mu_{i+1}$. Załóżmy, że hipotezy testowaliśmy niezależnie. Jeżeli żadna ze średnich z wybranej pary $\mu_l, \mu_{l+1}$ nie sąsiadowała z $\mu_i, \mu_{i+1}$ to $p$-value dla hipotezy o równości średnich $\mu_l, \mu_{l+1}$ nie powinno się zmienić. Zatem w każdym kroku na nowo powinniśmy liczyć jedynie dwa $p$-value. Jeżeli by nam się to udało, to liczba policzonych statystyk $t$/$p$-value w całym algorytmie będzie wynosić $k - 1 + 2(k - 2) = 3k - 3$ (liniowo).
...
Liczba policzonych statystyk $t$ w całym algorytmie: $k^2$.
