...
$$L(\sigma, \beta |y) \propto \left(\frac{RSS(\beta)}{n}\right)^{-n/2}.$$
...
$$\frac{L(\hat{\sigma}_X, \hat{\beta}_X |y)}{L(\hat{\sigma}_W, \hat{\beta}_W |y)} = \frac{RSS(\hat{\beta}_W)}{RSS(\hat{\beta}_X)}$$
...
******
Wracając do ANOVY i naszego przykładu. Możemy przetestować równość średnich na ulicach, używając formuły.
...
Testów *post hoc* jest wiele. 

### Test LSD Fishera

Dla każdej pary grup $i, j$ liczymy statystykę $t$

$$t = \frac{\mu_i - \mu_j}{s_p \sqrt{\frac{1}{n_i} + \frac{1}{n_j}}}, \;\;\;\;\;\; s_p = \sqrt{\frac{(n_i - 1)s_i^2 + (n_j - 1)s_j^2}{n_1 + n_2 - 2}}.$$
...
# Pytania badawcze 
 ...
 - Czy możemy stworzyć algorytm, którego wyniki będą zgodne (czyli otrzymane klastry będą rozłączne)? Jaka będzie złożoność takiego algorytmu?
